...
export class SocketIOWrapper {
  ...
  _init() {
    ...
    this.socketInstance.on(
      SocketIOWrapper.staticEvents[0],
      this.handleConnectEvent.bind(this)
    );
    this.socketInstance.on(
      SocketIOWrapper.staticEvents[1],
      this.handleErrorEvent.bind(this)
    );
    this.socketInstance.on(
      SocketIOWrapper.staticEvents[2],
  ...
  emit(eventName, data) {
    ...
    if (
      isEmpty(eventName) ||
      SocketIOWrapper.staticEvents.includes(eventName)
    ) {
